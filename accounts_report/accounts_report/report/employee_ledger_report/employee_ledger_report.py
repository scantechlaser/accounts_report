...
def execute(filters=None):
	...
	if 'iou_account_check' in filters:
		...
		my_sql = "SELECT * FROM `tabAccount` WHERE parent_account = 'Imprest - SLPL'"

		getEmployee = frappe.db.sql(my_sql, as_dict=True)
		if getEmployee:
			for i in getEmployee:

				filters['party_type'] = ''
				filters['party'] = ''
				filters['account'] = i.name
				validate_filters(filters, account_details)
				...
				if res:
					...
					if res[int(len(res))-1]['debit'] != 0:
						temp = True
					if res[int(len(res))-1]['credit'] != 0:
						temp = True
					if res[int(len(res))-1]['balance'] !=0:
						temp = True

					if temp:
						employee_name = str(i.name).split("-")
						# my_sql = "SELECT * FROM `tabEmployee` WHERE employee_name like '%"+(employee_name[1])+"%'"
						# getEmployee = frappe.db.sql(my_sql, as_dict=True)
						# if getEmployee:

						res[int(len(res))-1]['emloyee'] = ''
						res[int(len(res))-1]['account'] = i.name
						res[int(len(res))-1]['employee_name'] = employee_name[1]
						result.append(res[int(len(res))-1])

	if iou == False:
		my_sql = "SELECT * FROM `tabEmployee` "

		if 'party' in filters:

			my_sql += " WHERE name = '"+str(filters['party'])+"'"
		...
		if getEmployee:
			for i in getEmployee:
				...
				if res:
					...
					if temp:
						res[int(len(res))-1]['emloyee'] = i.name
						res[int(len(res))-1]['account'] = 'IOU - Employees - SLPL'
						res[int(len(res))-1]['employee_name'] = str(i.employee_name)
						result.append(res[int(len(res))-1])
	...
	for gle in gl_entries:
		if gle.posting_date < from_date or cstr(gle.is_opening) == "Yes":
			...
			if gle.account =="'Opening'":
				if gle.debit !=0.0 and gle.credit != 0.0:
					if filters.get("group_by_account"):
